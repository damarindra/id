...
;(function($) {
  ...
  SearchModal.prototype = {
    ...
    search: function(search) {
	  ...
	  var opts = function(data) {
	      var $inputSearch = document.getElementById('algolia-search-input');
	      ...
	      $inputSearch.addEventListener('keypress', function(e) {
		    ...
		    if (key === 13) { // 13 is enter
		      ...
		      var keywords = this.value.trim().toLowerCase().split(/[\s\-]+/);
      	        ...
      	        if (this.value.trim().length <= 0 || this.value.length <= 1) {
      	        var $listSearch = document.getElementById('list-search');
      	        $listSearch.innerHTML = "";
      	          return;
      	        }

      	        var results = self.filterPosts(data, keywords);
      	        var $listSearch = document.getElementById('list-search');
      	        $listSearch.innerHTML = self.createInnerHTML(results);
		    }
    ...
    filterPosts: function(data, keywords) {
    var results = [];
  ...
  createInnerHTML: function(results) {
	  var self = this;
    var content = '';
    results.forEach(function(item) {
      var postContent;
      postContent = self.highlightText(item.content, item.matchKeyWords);
      postContent = self.getPreviewContent(postContent, item.matchKeyWords);

      item.title = self.highlightText(item.title, item.matchKeyWords);

	item = '<div class="media">' +
	'<div class="media-body">' +
	'<a class="link-unstyled" href="' + item.url + '"" target="_blank">' +
	'<h3 class="media-heading">' + item.title +'</h3>' +
        '</a>' +
        '<div class="media-content hide-xs">' + postContent + '</div>' +
        '</div>';
/*
      item = '<li class="item">' +
  ...
  getPreviewContent: function (content, matchKeyWords) {
    var isMatch = false;
    var index = 0;
  ...
  highlightText: function (text, matchKeyWords) {
    text = text.replace(/<[^>]*>/g, '');
    matchKeyWords.forEach(function(word) {
...
;(function($){
  ...
  /*
	...
	$(document).ready(function() {
		//make sure if we are in post page
		if ($('.post-bottom-bar').length) {
